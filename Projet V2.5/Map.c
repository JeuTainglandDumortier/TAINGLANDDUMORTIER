...
void RenderMap(Map* map,SDL_Renderer* mapRend)
{
    ...
    for (int i = 0; i < map->hauteur; i++)
    {
        for (int j = 0; j < map->largeur; j++)
        {
			...
			//tabtexture[i][j]->dest.x=Rect_source.x
			//tabtexture[i][j]->dest.y=Rect_dest.y;
...
int compteur = 0;
 ...
 if ((file = fopen(tileset_path, "r")) == NULL)
...
/*FILE * monFichier;
tabcar=(char *) malloc(compteur * sizeof(char)); 
monFichier = fopen(tileset_path, "r");
fgets(tabcar, compteur, monFichier); 
return tabcar;
}*/
